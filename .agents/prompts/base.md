...
* 环境变量 `EVENT_NAME`：当前是哪种事件触发了本次任务，可能的值有：`issue_comment`, `pull_request_comment`, `issues`
* 环境变量 `EVENT_OBJECT_NUMBER`：对应的 Issue 号或是 PR 号
* 环境变量 `EVENT_COMMENT_ID`：触发本次运行的评论的数据库 ID（仅 `issue_comment` 或 `pull_request_comment` 事件时有效，`issues` 事件时为空字符串）
...
对于 `$EVENT_NAME` 为 `issues` 事件（`$EVENT_COMMENT_ID` 为空）：在 Issue 本身的第一条评论上（issue body）添加 `eyes` 反应：
...
对于 `$EVENT_NAME` 为 `issues` 事件（`$EVENT_COMMENT_ID` 为空），从 Issue 本身的第一条评论上（即 issue body）移除 `eyes`
...
对于 `$EVENT_NAME` 为 `issues` 事件（`$EVENT_COMMENT_ID` 为空），在 Issue 本身的第一条评论上（issue body）添加 `rocket` 反应：
